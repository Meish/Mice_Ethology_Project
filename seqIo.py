...
def tsSync(video_path, srTop, srFront):
    ...
    if seqV == 1:
        for f in range(len(tsDepth)):
            if (
                tsDepth[f] - np.floor(tsDepth[f]) >= 0.5
            ):  # Santiago's bug acquisistion software
                tsDepth[f] -= 1
        tsDepth -= 0.03  # substract the systematic timeshift

        # load front and top view and convert from UTC to PST
        hourShift = np.round((tsDepth[0] - tsTop[0]) / 3600.0) * 3600
        timeShift = tsDepth[0] - tsTop[0] - 0.066
...
def ts2str(ts):
    t = ts / 86400.0 + date.toordinal(date(1971, 1, 2))
    # datetime.fromtimestamp(t)
    str_time = (
        datetime.fromordinal(int(t)) + timedelta(days=t % 1) - timedelta(days=366)
    ).strftime("%Y-%m-%d %H:%M:%S") + ".%03d" % np.round((ts - np.floor(ts)) * 1000)
    return str_time
...
def parse_ann_dual(f_ann):
    ...
    ann_dict = {
        ...
        "behs_bout": actions,
        "behs_frame": action_frame
        if "interaction" not in action_frame
        else action_frame2,
        "behs_frame2": action_frame2
        if "interaction" in action_frame2
        else action_frame,
    }
...
def syncTopFront(f, num_frames, num_framesf):
    return (
        int(round(f / (num_framesf - 1) * (num_frames - 1)))
        if num_framesf > num_frames
        else int(round(f / (num_frames - 1) * (num_framesf - 1)))
    )
...
class seqIo_reader:
    ...
    def readHeader(self):
        ...
        if all(tmp == 0):
            raise ValueError("fully empty header")
        self.file.seek(0, 0)
        ...
        if not "{0:X}".format(magic_number) == "FEED" or not name == "Norpix seq":
        ...
        if self.imageFormat in (100, 200):
            self.ext = "raw"
        elif self.imageFormat in (102, 201):
            self.ext = "jpg"
        elif self.imageFormat in (0x001, 0x002):
            self.ext = "png"
        elif self.imageFormat == 101:
            self.ext = "brgb8"
        elif self.imageFormat == 103:
            self.ext = "jbrgb"
        else:
            raise ValueError("uknown format")

        self.compressed = (
            True if self.ext in ["jpg", "jbrgb", "png", "brgb8"] else False
        )
    ...
    def getTs(self, n=None):
        ...
        for i in range(n):
            if not self.compressed:  # uncompressed
                self.file.seek(
                    1024
                    + i * self.header["trueImageSize"]
                    + self.header["imageSizeBytes"],
                    0,
                )
            else:  # compressed
            ...
            # print(i)
            ts[i] = (
                fread(self.file, 1, np.uint32) + fread(self.file, 1, np.uint16) / 1000.0
            )
    ...
    def getFrame(self, index, decode=True):
        ...
        if self.ext in [
            ...
            assert False
        ts = fread(self.file, 1, np.uint32) + fread(self.file, 1, np.uint16) / 1000.0
...
class seqIo_writer:
    def __init__(self, filename, old_header):
        ...
        if codec in ["monoraw", "imageFormat100"]:
            self.frmt = 100
            self.nch = 1
            self.ext = "raw"
        elif codec in ["raw", "imageFormat200"]:
            self.frmt = 200
            self.nch = 3
            self.ext = "raw"
        elif codec in ["monojpg", "imageFormat102"]:
            self.frmt = 102
            self.nch = 1
            self.ext = "jpg"
        elif codec in ["jpg", "imageFormat201"]:
            self.frmt = 201
            self.nch = 3
            self.ext = "jpg"
        elif codec in ["monopng", "imageFormat001"]:
            self.frmt = 0x001
            self.nch = 1
            self.ext = "png"
        elif codec in ["png", "imageFormat002"]:
            self.frmt = 0x002
            self.nch = 3
            self.ext = "png"
        else:
    ...
    def writeHeader(self):
        ...
        if (
            not "descr" in self.header.keys()
            or len(np.array([self.header["descr"]]).view(np.uint8)) > 256
        ):
            d = np.array(["No Description"]).view(np.uint8)
        else:
            d = np.array([self.header["descr"]]).view(np.uint8)
        d = np.concatenate(
            (d[: np.minimum(256, len(d))], np.zeros(256 - len(d)).astype(np.uint8))
        )
        ...
        vals = [
            self.header["width"],
            self.header["height"],
            self.header["imageBitDepth"],
            self.header["imageBitDepthReal"],
            self.header["imageSizeBytes"],
            self.header["imageFormat"],
    ...
    def addFrame(self, I, ts=0, encode=1):
        ...
        if ext == "raw":
            ...
            if not encode:
                assert I.size == self.header["imageSizeBytes"]
            else:
                if nCh == 3:
                    t = I[:, :, 2]
                    I[:, :, 2] = I[:, :, 0]
                ...
                if nCh == 1:
                    ...
                    I = np.transpose(np.expand_dims(I, axis=2), (2, 1, 0))
        ...
        if ext == "jpg":
            if encode:
                ...
                fid.close()
                b = bytearray(I)
                assert b[0] == 255 and b[1] == 216 and b[-2] == 255 and b[-1] == 217
                # JPG
                os.remove("tmp.jpg")
...
def seqIo_frImgs(
    fName, header=[], aviName=[], Is=[], sDir=[], name="I", ndig=5, f0=0, f1=1e6
):
    """
...
class seqIo_dualReader:
    ...
    def __init__(self, file1, file2):
        ...
        self.info["width"] = np.maximum(
            self.s1.header["width"], self.s2.header["width"]
        )
